...
使用已裁剪的图片位置进行实验  结果一直不好

还是继续使用人脸框检测吧 至少下载的特征文件大小一致

#### 继续使用retinaface和san算法

换回原来的算法之后，裁剪时将有问题的路径也进行更改
...
使用cv2自带的光流算法，但是不使用GPU可能很慢

先试一下，看看需要多长时间

使用所有帧作为已裁剪的图片先看看需要花多长时间
...
1.使用较低版本的opencv-contrib-python-headless
...
> ```
> Traceback (most recent call last):
>   File "/kaggle/working/ME-GCN-Project/feature_extraction/cas(me)^2/new_crop.py", line 4, in <module>
>     import cv2
...
> ImportError: /opt/conda/bin/../lib/libcairo-gobject.so.2: undefined symbol: g_memdup2
> ```

glib版本太低导致的 不支持g_memdup2 我的想法是 是不是cmake的版本太高 使用2.16的版本没有遇到这样的问题
...
我感觉确实是符号链接的问题，因为我同时编译opencv和denseflow而不是使用已编译好的文件之后，python文件好像使用的是我编译的cv，但是还不确定，得再测试下，因为不知道时候还有别的问题

进行这样的配置后还是没法解决问题

其实我还有一种想法，就是不升级cmake的版本看看有没有问题

但是cv2调用的话可能也还是有问题

好像和cmake的版本没有关系，主要是编译到快完成时，服务器会主动将session断开，导致无法继续实验

但是我觉得将编译好的opencv移到别处有关，可能这样denseflow就失去了对应的功能
...
如果不用denseflow而是opecv gpu提取光流效果应该和使用cpu一样
...
使用denseflow2万多张图片40多分钟就能完成，但是光流提取的效果不太好

所以现在只期待使用gpu版本的opencv提取的时间能够减少

#### 使用cpu提取光流特征

大概需要1百多个小时

使用三个浏览器，每个浏览器大概开6个cpu session

只要每次运行的时间不超过12小时就没事，反正使用的是cpu

每次最多使用5个cpu

有-2的会无法运行，不知是什么原因，是不是同一环境的原因？
...
这段代码是将处于[0,255]的光流值归一化，然后中心化到[-0.5,0.5]

但是进行这段处理之后，光流值并没有处于[-0.5,0.5]之间
...
flow_x_y = np.stack((flow_x, flow_y), axis=-1)
    ...
    # 将0~255范围的光流归一化 并将中心移到0 即范围变为[-0.5, 0.5]之间
    flow_x_y1 = flow_x_y / np.float32(255) - 0.5
    print(np.min(flow_x_y1), np.max(flow_x_y1))
    epsilon = 1e-8  # 防止除零
    flow_x_y = (flow_x_y - min_val) / (max_val - min_val + epsilon) - 0.5
    print(np.min(flow_x_y), np.max(flow_x_y))
```

输出为

> ```bash
> -2.4313369 0.87644786
> -0.50953466 -0.49656296
> -0.5 0.5
>   0%|                                           | 1/659 [00:00<10:28,  1.05it/s]测试
> -1.2483697 0.90697837
> -0.50489557 -0.4964432
> -0.5 0.5
>   0%|▏                                          | 2/659 [00:01<10:17,  1.06it/s]测试
> -0.72509634 0.40296334
> -0.5028435 -0.49841976
> -0.5 0.5
>   0%|▏                                          | 3/659 [00:02<10:24,  1.05it/s]测试
> -0.9155189 1.169638
> -0.5035903 -0.49541318
> -0.5 0.5
>   1%|▎                                          | 4/659 [00:04<11:17,  1.03s/it]测试
> -0.6223707 0.75968754
> -0.5024407 -0.49702084
> -0.5 0.5
> ```



roi区域问题 casme_030_0505 17个nose_roi_left > nose_roi_right

> ```bash
>  31%|███████████▎                         | 2899/9434 [44:35<1:47:49,  1.01it/s]nose_roi_left > nose_roi_right
>  31%|███████████▎                         | 2900/9434 [44:36<1:47:24,  1.01it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2901/9434 [44:37<1:47:33,  1.01it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2902/9434 [44:38<1:46:02,  1.03it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2903/9434 [44:39<1:44:19,  1.04it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2904/9434 [44:40<1:44:09,  1.04it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2905/9434 [44:41<1:42:29,  1.06it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2906/9434 [44:42<1:39:39,  1.09it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2907/9434 [44:43<1:39:17,  1.10it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2918/9434 [45:01<1:46:56,  1.02it/s]nose_roi_left > nose_roi_right
>  31%|███████████▍                         | 2929/9434 [45:28<1:55:29,  1.07s/it]nose_roi_left > nose_roi_right
>  31%|███████████▌                         | 2940/9434 [45:54<1:55:28,  1.07s/it]nose_roi_left > nose_roi_right
>  31%|███████████▌                         | 2951/9434 [46:20<1:54:33,  1.06s/it]nose_roi_left > nose_roi_right
>  31%|███████████▌                         | 2962/9434 [46:47<1:55:36,  1.07s/it]nose_roi_left > nose_roi_right
>  32%|███████████▋                         | 2973/9434 [47:14<1:53:35,  1.05s/it]nose_roi_left > nose_roi_right
>  32%|███████████▋                         | 2984/9434 [47:41<1:55:11,  1.07s/it]nose_roi_left > nose_roi_right
>  32%|███████████▋                         | 2995/9434 [48:07<2:07:50,  1.19s/it]nose_roi_left > nose_roi_right
> 100%|█████████████████████████████████████| 9434/9434 [3:13:54<00:00,  1.23s/it]
> ```
...
> ```python
> scaled_magnitudes = (magnitudes - min_magnitude) / (max_magnitude - min_magnitude)  # 先缩放到 [0, 1]
> scaled_magnitudes = scaled_magnitudes - 0.5  # 平移到 [-0.5, 0.5]
> ```
...
提取原始光流，保存为float32类型，对其进行归一化，同时缩放到[0, 0.5]的范围内，加上幅度和方向，所有光流值范围在[-0.5,0.5]之间
...
这段是使用denseflow提取光流并分别保存x分量和y分量为灰度图像，然后进行归一化。

-0.5这个操作我认为是，将范围从[0,1]平移到[-0.5,0.5]之间。

因为保存灰度图和归一化都没有涉及到负数，即没有方向。

猜测-0.5是为了让光流值具有方向，但这样做，没法恢复光流得原始方向。

于是使用下面得代码，代码分别计算幅度和方向

但是有缩放的操作，这是为了将模仿将光流的幅值移到[-0.5, 0.5]之间
...
这样对光流进行调整后，效果有一点改善，但是离原来的效果还是很远

> ```
> Micro result: TP:0.0, FP:3.0, FN:57.0
> Precision =  0.0
> Recall =  0.0
> F1-Score =  0
> Macro result: TP:28.0, FP:294.0, FN:272.0
> Precision =  0.087
> Recall =  0.0933
> F1-Score =  0.09
> Total result: TP:28.0, FP:297.0, FN:329.0
> Precision =  0.0862
> Recall =  0.0784
> F1-Score =  0.0821
> ```
...
> ```
> Micro result: TP:1.0, FP:29.0, FN:56.0
> Precision =  0.0333
> Recall =  0.0175
> F1-Score =  0.023
> Macro result: TP:31.0, FP:350.0, FN:269.0
> Precision =  0.0814
> Recall =  0.1033
> F1-Score =  0.091
> Total result: TP:32.0, FP:379.0, FN:325.0
> Precision =  0.0779
> Recall =  0.0896
> F1-Score =  0.0833
> ```

这里我不能理解，到底是平移改变了，还是缩放改变了

但是检测到了微表情，而且宏表情的检测效果也不错，结合后面实验的结果

光流的数值要小，光流值要有种对称的感觉，这样效果会不错。

进行以下实验

1.不缩放，直接将幅度平移到[-0.5, 0.5]
...
> ```
> Micro result: TP:0.0, FP:10.0, FN:57.0
> Precision =  0.0
> Recall =  0.0
> F1-Score =  0
> Macro result: TP:25.0, FP:379.0, FN:275.0
> Precision =  0.0619
> Recall =  0.0833
> F1-Score =  0.071
> Total result: TP:25.0, FP:389.0, FN:332.0
> Precision =  0.0604
> Recall =  0.07
> F1-Score =  0.0649
> ```
...
```python
    if max_magnitude > min_magnitude:
        # 将幅度缩放到 [0, 1]，然后再缩放到 [0, 0.5]
        scaled_magnitudes = (magnitudes - min_magnitude) / (max_magnitude - min_magnitude)
        scaled_magnitudes = scaled_magnitudes * 0.25
    else:
        # 如果 max_magnitude 和 min_magnitude 相等，设为 0
        scaled_magnitudes = np.zeros_like(magnitudes)
...
> ```
> Micro result: TP:0.0, FP:5.0, FN:57.0
> Precision =  0.0
> Recall =  0.0
> F1-Score =  0
> Macro result: TP:28.0, FP:288.0, FN:272.0
> Precision =  0.0886
> Recall =  0.0933
> F1-Score =  0.0909
> Total result: TP:28.0, FP:293.0, FN:329.0
> Precision =  0.0872
> Recall =  0.0784
> F1-Score =  0.0826
> ```
...
def scale_flow_to_range(flow):
    """
    将原始光流的幅度缩放到 [-0.5, 0.5] 范围，不保留方向信息。

    Args:
        flow: np.ndarray, shape of flow should be (-1, 2)

    Returns:
        scaled_magnitudes: 缩放并平移后的光流幅度值
    """
    assert flow.dtype == np.float32, "Element type of optical flow should be float32"

    # Step 1: 计算每个光流向量的幅度
    magnitudes = np.linalg.norm(flow, axis=1, keepdims=True)  # 计算光流幅度 (n, 1)
    magnitudes = np.clip(magnitudes, 1e-6, None)  # 避免出现零值，防止除零错误

    # Step 2: 找到最大和最小的幅度值
    max_magnitude = np.max(magnitudes)
    min_magnitude = np.min(magnitudes)

    # Step 3: 对幅度进行归一化并映射到 [-0.5, 0.5] 范围
    if max_magnitude > min_magnitude:
        # 将幅度归一化到 [0, 1]，再映射到 [-0.5, 0.5]
        scaled_magnitudes = (magnitudes - min_magnitude) / (max_magnitude - min_magnitude)
        scaled_magnitudes = scaled_magnitudes * 1.0 - 0.5  # 映射到 [-0.5, 0.5]
    else:
        # 如果 max_magnitude 和 min_magnitude 相等，设为 0
        scaled_magnitudes = np.zeros_like(magnitudes)
...
4.先将光流归一化到[0,255]，再进一步归一化到[0,1]然后再平移到[-0.5,0.5]
...
def normalize_flow_to_range(flow):
    ...
    # Step 2: 获取光流的最大值
    max_value = np.max(np.abs(flow))  # 取绝对值最大值，确保正负对称

    if max_value == 0:
        raise ValueError("Max value of the flow is zero, normalization not possible.")

    # Step 3: 将光流归一化到 [0, 255]
    flow_x_normalized_255 = ((flow_x + max_value) / (2 * max_value)) * np.float32(255)
    flow_y_normalized_255 = ((flow_y + max_value) / (2 * max_value)) * np.float32(255)
...
> ```
> Micro result: TP:0.0, FP:1.0, FN:57.0
> Precision =  0.0
> Recall =  0.0
> F1-Score =  0
> Macro result: TP:10.0, FP:70.0, FN:290.0
> Precision =  0.125
> Recall =  0.0333
> F1-Score =  0.0526
> Total result: TP:10.0, FP:71.0, FN:347.0
> Precision =  0.1235
> Recall =  0.028
> F1-Score =  0.0457
> ```

5.将获取光流的最大值改为获取光流模的最大值
...
> Recall =  0.0
> F1-Score =  0
> Macro result: TP:8.0, FP:81.0, FN:292.0
> Precision =  0.0899
> Recall =  0.0267
> F1-Score =  0.0411
> Total result: TP:8.0, FP:81.0, FN:349.0
> Precision =  0.0899
> Recall =  0.0224
> F1-Score =  0.0359
> ```

6.提取的特征值与仓库提供的特征值相比范围更大，波动更大
...
```python
def normalize_flow_to_range(flow, kernel_size=5, sigma=1.0):
    """
    将光流的 x 和 y 分量归一化到 [0, 1]，然后平移到 [-0.5, 0.5] 范围，同时进行空间平滑。

    Args:
        flow: 光流数据, np.ndarray, shape of flow should be (H, W, 2)
        kernel_size: 高斯滤波器的核大小
        sigma: 高斯滤波器的标准差
    ...
    # Step 2.1: 对光流的 x 和 y 分量进行高斯空间平滑
    flow_x = cv2.GaussianBlur(flow_x, (kernel_size, kernel_size), sigma)
    flow_y = cv2.GaussianBlur(flow_y, (kernel_size, kernel_size), sigma)
...
> Recall =  0.0
> F1-Score =  0
> Macro result: TP:9.0, FP:108.0, FN:291.0
> Precision =  0.0769
> Recall =  0.03
> F1-Score =  0.0432
> Total result: TP:9.0, FP:108.0, FN:348.0
> Precision =  0.0769
> Recall =  0.0252
> F1-Score =  0.038
> ```

感觉是光流的问题，但是怎么测都没有好的结果，暂时先停下。

提取的光流用于提取特征，提取的特征范围太大，原特征提供的范围较小。

是不是要取光流模的总和，但是取光流模的总和可能导致光流的过度压缩

不过在没取总和之前，取最大值时，提取的特征范围偏大，几乎大了十倍以上

所以怀疑是不是使用了光流模对光流进行收敛和压缩才使得范围变得集中
...
def normalize_flow_to_range(flow):
    ...
    # Step 2: 计算每个像素的光流模，并获取模的总和
    magnitudes = np.linalg.norm(flow, axis=2)  # 计算光流的模 (H, W)
    sum_magnitude = np.sum(magnitudes)  # 取光流模的总和

    if sum_magnitude == 0:
        raise ValueError("Sum magnitude of the flow is zero, normalization not possible.")

    # Step 3: 将光流的 x 和 y 分量归一化到 [0, 255]
    # (2 * sum_magnitude) 为之后-0.5做准备 使得正负对称
    flow_x_normalized_255 = ((flow_x + sum_magnitude) / (2 * sum_magnitude)) * np.float32(255)
    flow_y_normalized_255 = ((flow_y + sum_magnitude) / (2 * sum_magnitude)) * np.float32(255)
...
> Recall =  0.0
> F1-Score =  0
> Macro result: TP:5.0, FP:50.0, FN:295.0
> Precision =  0.0909
> Recall =  0.0167
> F1-Score =  0.0282
> Total result: TP:5.0, FP:50.0, FN:352.0
> Precision =  0.0909
> Recall =  0.014
> F1-Score =  0.0243
> ```
...
def normalize_flow_to_range(flow):
    ...
    # Step 2: 分别计算 x 和 y 向量的绝对值之和
    abs_sum_x = np.sum(np.abs(flow_x))  # x 分量的绝对值之和
    abs_sum_y = np.sum(np.abs(flow_y))  # y 分量的绝对值之和

    # 如果 x 和 y 的绝对值之和为 0，归一化不可行
    if abs_sum_x == 0 or abs_sum_y == 0:
        raise ValueError("The sum of absolute values for flow_x or flow_y is zero, normalization not possible.")

    # Step 3: 将光流的 x 和 y 分量归一化到 [0, 255]
    # 使用 x 和 y 的绝对值之和进行归一化
    flow_x_normalized_255 = ((flow_x + abs_sum_x) / (2 * abs_sum_x)) * np.float32(255)
    flow_y_normalized_255 = ((flow_y + abs_sum_y) / (2 * abs_sum_y)) * np.float32(255)
...
> Recall =  0.0
> F1-Score =  0
> Macro result: TP:0.0, FP:63.0, FN:300.0
> Precision =  0.0
> Recall =  0.0
> F1-Score =  0
> Total result: TP:0.0, FP:63.0, FN:357.0
> Precision =  0.0
> Recall =  0.0
> F1-Score =  0
> ```
...
roi_flows应该是和global_optflow_vector相似的

global_optflow_vector最后进行了平均归一化处理

roi_flows取处理好的原始光流的一些区域，所以对原始光流的处理可能和global_optflow_vector的处理相似
